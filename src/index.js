...
const STUDY_MESSAGES = {
  USER_MATHY:
    'GET OFF OF DISCORD :point_right: :regional_indicator_a: :regional_indicator_n: :regional_indicator_k: :regional_indicator_i:',
  USER_TAME:
    "Stay focussed you muppet.\n> i want to be reminded that i'm doing this because i hate my job"
}
...
client.on('message', (msg) => {
  ...
  if (
    getters.getIsStudying(msg.author.id) &&
    msg.content !== '!study disable'
  ) {
    const user = Object.keys(process.env).find(
      (key) => process.env[key] === msg.author.id
    )
    msg.reply(
      STUDY_MESSAGES[user] || 'you should be studying <:grr:765369824762527774>'
    )
    return
  }
