...
const corrupt = (word) => {
  ...
  const randomNumber = generateRandomNumberUpTo(13)
...
const getUnkifiedWords = (words, timesToRepeat) => {
  if (timesToRepeat <= 0) {
    return words
  } else {
    return getUnkifiedWords(words.map(unkify), timesToRepeat - 1)
  }
}

module.exports = (msg, args) => {
  const unkiePrefix = '<:unkie:742854278930104355>:mega: '
  const [timesToRepeat, ...words] = args
  if (timesToRepeat > 100) {
    const unkifiedWords = getUnkifiedWords(words, 100)
    msg.channel.send(unkiePrefix + unkifiedWords.join(' ') || '_ _')
  } else {
    const unkifiedWords = getUnkifiedWords(words, timesToRepeat)
    msg.channel.send(unkiePrefix + unkifiedWords.join(' ') || '_ _')
  }
}
