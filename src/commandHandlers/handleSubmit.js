...
module.exports = (msg, args) => {
  if (
    args.length === 1 &&
    (args[0].toLowerCase() === 'egg' || args[0].toLowerCase() === 'egg.')
  ) {
    const eggs = getters.getEggs()
    const newEggs = eggs + 1
    actions.setEggs(newEggs)
    msg.channel.send(
      `Egg submitted! (${newEggs} ${newEggs === 1 ? 'egg' : 'eggs'} submitted)`
    )
    ...
    if (msg.author.id === process.env.USER_EB) {
      ...
      msg.channel.send(
        'This command is for resetting eggs after a bot reset and is for the use of [eb] only.'
      )
    }
  } else {
